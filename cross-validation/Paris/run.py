...
GAN_MODEL_BEST_FILE = '模型/gan_best.model_2048-2048 0.00005 0.00008 epoch=10 55 TEST 1x1cov R_mac+RA'
...
def main():
 ...
 # test
    ...
    generator_best = GEN(FEATURE_SIZE, G_WEIGHT_DECAY, G_LEARNING_RATE, Oxford_LEN_UNSIGNED, param=param_best)
    sess = tf.Session(config=config)
    sess.run(tf.initialize_all_variables())
    map_best = MAP_test(sess, generator_best, query_test_feature, query_test_label, pred_label, pred_feature, QUERY_TEST_SIZE, Oxford_LEN_UNSIGNED,LEN_GAN)
