...
class DIS():
    def __init__(self, feature_size, weight_decay, learning_rate, len_gan, loss= 'softmax', param=None):
        ...
        self.gan_data = tf.placeholder(tf.float32, shape=[self.len_gan, 2048], name="gan_data")
        ...
        gan_vector = tf.reshape(tf.nn.relu(self.conv2d(tf.reshape(self.gan_data,[self.len_gan,1,1,2048]), self.W_conv) + self.b_conv),[self.len_gan,self.feature_size])
        ...
        if loss == 'softmax':
            ...
            with tf.name_scope('softmax_loss'):
                #self.loss = tf.reduce_mean(tf.log(tf.clip_by_value(tf.nn.relu(self.pred_score-self.neg_score),1e-8,1.0))) + self.weight_decay * (tf.nn.l2_loss(self.W_conv) + tf.nn.l2_loss(self.b_conv))
                #self.reward = tf.reshape(tf.log(tf.clip_by_value(tf.nn.relu(self.pred_score-self.neg_score),1e-8,1.0)), [-1])
                self.loss = -tf.reduce_mean(tf.log(tf.nn.relu(self.pred_score-self.neg_score))) + self.weight_decay * (tf.nn.l2_loss(self.W_conv) + tf.nn.l2_loss(self.b_conv))
                self.reward = tf.reshape(tf.log(tf.nn.relu(self.pred_score-self.neg_score)), [-1])
        else:
